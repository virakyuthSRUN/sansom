...
const getColor = (s: number) =>
  s <= 30 ? "#00c896" : s <= 60 ? "#ffb300" : "#ff4757";
const getLabel = (s: number) =>
  s <= 30 ? "LOW RISK" : s <= 60 ? "MEDIUM RISK" : "HIGH RISK";
...
const DebtGoalsPage = () => {
  ...
  const simulate = () => {
    // Simulator inputs are in display currency, convert to MYR for ratio
    const i = (parseFloat(checked.income) || 0) / currency.rate;
    const bn = (parseFloat(checked.bnpl) || 0) / currency.rate;
    const lo = (parseFloat(checked.loans) || 0) / currency.rate;
    if (i <= 0) return;
  ...
  const handleDeposit = (id: string) => {
    // depositAmount is in display currency (e.g. MYR/USD/KHR/IDR)
    const amtInDisplayCurrency = parseFloat(depositAmount);
    if (!amtInDisplayCurrency || amtInDisplayCurrency <= 0) return;
    const goal = goals.find((g) => g.id === id);
    if (!goal) return;

    // Convert back to MYR (base) before storing
    const amtInMYR = amtInDisplayCurrency / currency.rate;

    const currentSaved = Number(goal.saved);   // stored in MYR
    const currentTarget = Number(goal.target); // stored in MYR
    const remaining = currentTarget - currentSaved;
    const actualDeposit = Math.min(amtInMYR, remaining);
    const newSaved = parseFloat((currentSaved + actualDeposit).toFixed(2));
  ...
  // remaining in display currency for the warning check
  const remainingInDisplay = (g: Goal) =>
    (g.target - g.saved) / currency.rate;

  const totalSaved = goals.reduce((acc, g) => acc + Number(g.saved), 0);
  const totalTarget = goals.reduce((acc, g) => acc + Number(g.target), 0);
  // monthlySuggested is in MYR base
      ...
      {tab === "debt" ? (
        <>
          <div
            ...
            style={{
              background: getBg(data.debtScore),
              borderColor: `${getColor(data.debtScore)}30`,
            }}
          >
            <RingChart
              pct={data.debtScore}
              size={96}
              color={getColor(data.debtScore)}
              bg="#e8e8e8"
              stroke={9}
              label={`${data.debtScore}`}
              sub="/ 100"
            />
            <div className="flex-1 text-center sm:text-left">
              ...
              <p
                ...
                style={{ color: getColor(data.debtScore) }}
              >
                {getLabel(data.debtScore)}
              </p>
              <p className="text-xs text-foreground leading-relaxed mt-1.5">
                You have {data.bnplCount} active BNPL plans. Your
                debt-to-income ratio is <b>25%</b> — approaching the danger
                zone of 30%.
              </p>
            </div>
          </div>
          ...
          <div className="bg-card rounded-2xl p-4 shadow-sm">
            ...
            <p className="text-[11px] text-muted-foreground mb-3.5">
              Enter your details to get a personalised risk score
            </p>
            <div className="grid grid-cols-2 gap-2.5 mb-3.5">
              {[
                [`Monthly Income (${currency.symbol})`, "income", "e.g. 1500"],
                [
                  `Monthly Savings (${currency.symbol})`,
                  "savings",
                  "e.g. 300",
                ],
            ...
            <button
              onClick={simulate}
              className="w-full gradient-primary text-primary-foreground rounded-xl py-3.5 font-semibold text-sm shadow-primary hover:shadow-primary-hover transition-all"
            >
            ...
            {simScore !== null && (
              <div
                ...
                style={{
                  background: getBg(simScore),
                  borderColor: `${getColor(simScore)}30`,
                }}
              >
                <p className="text-[11px] text-muted-foreground font-semibold mb-1">
                ...
                <p
                  ...
                  style={{ color: getColor(simScore) }}
                >
                  {simScore}/100 — {getLabel(simScore)}
                </p>
        ...
        <>
          ...
          {goals.map((g) => {
            ...
            // months based on MYR base
            const months =
              remainingMYR > 0
                ? Math.ceil(remainingMYR / monthlySuggested)
                : 0;
